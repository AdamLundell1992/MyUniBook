        ...
        @foreach($users as $user)
            ...
            <div class="row pb-5 m-lg-5 mt-5">
                ...
                <div class="col-md-7 col-sm-7">
                    <h5 class="fw-bold" style="font-size: x-large">{{$user->name}}</h5>
                    <p class="fw-bold" style="font-size: x-large">{{$user->UniCourse}}</p>
                    <p class="fw-bold" style="font-size: x-large">{{$user->email}}</p>
                    {{--                <div class="col-md-3 col-sm-3">--}}
                    <div class="row mt-2">
                        <div class="col-4">
                            <a href="{{url('confirmRequest' . $user->id . '/')}}" class="profile-link">
                                <button class="btn btn-primary btn-sm "><i class="fas fa-user-check"
                                                                           style="font-size: x-large"></i> Accept friend
                                </button>
                            </a>
                        </div>
                        <div class="col-5">
                            <a href="{{url('deleteRequest' . $user->id . '/')}}" class=" profile-link">
                                <button class="btn btn-primary btn-sm"><i class="fas fa-trash-alt"
                                                                          style="font-size: x-large"></i> Decline Friend
                                    request
                                </button>
                            </a>
                        </div>
